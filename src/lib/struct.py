...
# http://www.brython.info/gallery/debug/Lib/_struct.py, originally
# https://bitbucket.org/pypy/pypy/src/18626459a9b2/lib_pypy/_struct.py?at=py3k-listview_str
# from PyPy's pure Python 'struct' implementation. With bug fixes from Brython
# and python3->python2 conversion
...
def unpack_float(data, index, size, le):
    binary = [ord(data[i]) for i in range(index, index + size)]
    if le == "big":
        binary.reverse()
    unsigned = 0
    for i in range(size):
    ...
    while i<len(fmt):
        ...
        if cur == 'x':
            ...
            if isinstance(args[0], bytes):
                ...
                if padding > 0:
                    result += [_bytes([len(args[0])]) + args[0][:num-1] + '\0'*padding]
                else:
                    if num<255:
                        result += [_bytes([num-1]) + args[0][:num-1]]
                    else:
                        result += [_bytes([255]) + args[0][:num-1]]
                args.pop(0)
            else:
                raise StructError("arg for string format not a string")
